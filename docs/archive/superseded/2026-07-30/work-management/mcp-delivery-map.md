...
> 單一 `smart-living-butler` MCP Server，共用同一份 45-tool Registry。
...
| `order` | `create_order`、`list_my_orders` 與六項客服工具 | SQLite 訂單、規則計價、客服所有權、SLA 與工單事件完成；品牌履約回呼待辦 | **已交付（品牌回呼待接）** |
| `community` | 團購工具；聯合服務的查詢、建稿、發布、匿名加入、截止成案、指派、開工與完工 9 項工具 | 團購與聯合服務 SQLite 狀態機、匿名需求去重、兩案比較、廠商所有權、指派與履約事件完成 | **已交付** |
...
| `backoffice` | `list_support_queue`、`start_support_ticket`、`resolve_support_ticket`；其餘 connector、方案與推播待辦 | 客服處理佇列完成；connector service 待辦 | **客服已交付；其餘待包裝** |
...
4. 將單一競賽社區 scope 換成正式租戶 claim 與廠商身分對應。
