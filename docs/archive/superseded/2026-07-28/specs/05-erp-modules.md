...
> ⚠️ **入口結構已由 [ADR-0015](../adr/0015-role-separated-entry-points.md) 修正**：改為
> `/login` 選擇身分後進入 `/user`、`/admin`、`/partner` 各自的介面，**不再有工作區切換器**；
> 平台整合（`/platform`）依決定屬後續加分，不列入登入選項與主導覽。
> 本文各工作區的**功能與權限內容仍然有效**，僅入口與導覽方式改變。
>
> 各角色共用統一服務 API，AI 功能沿用當前身分的權限，寫入操作必須預覽並確認。
