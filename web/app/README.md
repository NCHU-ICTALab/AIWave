...
- `/demo/resident`：住戶王小明的日光森林社區首頁，包含公告、包裹、報修、設備保養、公設預約、問社區與服務優惠。
...
登入後從右上角「Demo 角色切換」選單選擇「住戶王小明」或「主委陳建華」。選擇「重設 Demo」會清除本次開團、跟團、問答與未回答問題，回到住戶初始狀態並前往 `/demo/resident`。程式上也可呼叫 `useCommunityDemoStore().resetDemo()`。

### 九步簡報走查

1. 從 `/demo/resident` 先看 2 件待領包裹、電梯保養公告、報修進度與進行中團購。
